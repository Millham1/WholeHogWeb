...
# 1) Ensure a Build Report button exists at the bottom
...
# 3) Inject a <script src="wh-report.js"> include just before </body> (if not already present)
if ($updated -notmatch 'src=["''][^"']*wh-report\.js["'']') {
  ...
  if ($updated -match '(?is)</body\s*>\s*</html\s*>') {
    ...
    Write-Host "⚠ Could not find </body></html> to add the script include. Add this line manually near the end:" -ForegroundColor Yellow
