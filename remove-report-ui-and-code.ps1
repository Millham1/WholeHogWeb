...
# 1) Remove the build-report containers/buttons (both versions), if present
#    These are very targeted by ID so nothing else is touched.
$patterns = @(
...
# 2) Remove inline report logic <script id="wh-build-report-logic">...</script>
$patLogic = '\s*<script[^>]*\bid\s*=\s*"wh-build-report-logic"[^>]*>.*?</script>\s*'
$new = RxReplace $updated $patLogic
if ($new -ne $updated) { $updated = $new; $changed = $true }

# 3) Remove external include <script id="wh-report-js" src="wh-report.js">...</script> (any src containing wh-report.js)
$patInclude = '\s*<script[^>]*\bsrc\s*=\s*"[^"]*wh-report\.js"[^>]*>\s*</script>\s*'
$new = RxReplace $updated $patInclude
if ($new -ne $updated) { $updated = $new; $changed = $true }

# 4) Write the cleaned HTML back
if ($changed) {
