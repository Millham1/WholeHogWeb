...
if (-not (Test-Path -LiteralPath $Leaderboard)) { throw "File not found: $Leaderboard" }

# Resolve paths
$abs  = (Resolve-Path -LiteralPath $Leaderboard).Path
...
# Ensure script include present (non-destructive)
if ($updated -notmatch '(?is)wh-report\.js') {
  if ($updated -match '(?is)</body\s*>\s*</html\s*>') {
    ...
    Write-Host "⚠ Could not find </body></html> to add the script include. Add this near the end:" -ForegroundColor Yellow
...
# Write hardened JS that wipes old listeners and intercepts clicks at capture
...
(function(){
  ...
  // Export for inline calls too
  window.buildGroupedReportCSV = buildGroupedReportCSV;

  // --- Hardening: neutralize prior handlers ---
  function sanitizeButton(){
    ...
    // Replace node with a fresh clone to wipe old listeners
    const clone = btn.cloneNode(true);
    clone.id = 'wh-build-report-btn';
    if (btn.parentNode) btn.parentNode.replaceChild(clone, btn);
    return clone;
  }

  function bind(){
    const btn = sanitizeButton() || document.getElementById('wh-build-report-btn');

    // Capture-phase interceptor: stops ANY other delegated/bubbling listeners
    document.addEventListener('click', function(e){
      const t = e.target;
      if (t && t.id === 'wh-build-report-btn') {
        e.preventDefault();
        e.stopPropagation();
    ...
    // Also attach a normal listener on the button (redundant but harmless)
    if (btn && !btn._whBound) {
...
Write-Host "✓ Wrote hardened wh-report.js" -ForegroundColor Green
