...
# Resolve absolute paths (robust on all PS)
...
# 1) Write/overwrite the good builder in wh-report.js (the same code you verified in Console)
...
(function(){
  ...
  // Export so inline onclick can call it directly
  window.buildGroupedReportCSV = buildGroupedReportCSV;
})();
'@
Set-Content -LiteralPath $jsPath -Value $js -Encoding UTF8
Write-Host "✓ Wrote wh-report.js" -ForegroundColor Green

# 2) Ensure <script src="wh-report.js"> is included once before </body>
if ($updated -notmatch '(?is)wh-report\.js') {
  if ($updated -match '(?is)</body\s*>\s*</html\s*>') {
    ...
    $changed = $true
    Write-Host '✓ Injected <script src="wh-report.js">' -ForegroundColor Green
  } else {
    Write-Host "⚠ Couldn't find </body></html>. Add this line near the end:" -ForegroundColor Yellow
...
# 3) Append a NEW link (not a button) that calls the builder and hard-stops event bubbling.
#    Styling is inline to look like your red buttons, but it has NO shared classes to avoid old delegated handlers.
if ($updated -notmatch 'id="wh-build-report3-link"') {
  $link = @'
<div id="wh-build-report3-container" style="width:100%;display:flex;justify-content:center;margin:12px 0 36px;">
  <a id="wh-build-report3-link"
     href="#"
     onclick="event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); return window.buildGroupedReportCSV();"
     style="display:inline-block; text-align:center; min-width:280px; padding:10px 14px; border-radius:10px; border:2px solid #000;
            background:#e53935; color:#000; font-weight:800; text-decoration:none; cursor:pointer;">
    Build Detailed Report (CSV)
  </a>
</div>
'@
  if ($updated -match '(?is)</body\s*>\s*</html\s*>') {
    $updated = [regex]::Replace($updated, '(?is)</body\s*>\s*</html\s*>', ($link + "`r`n</body></html>"), 1)
    $changed = $true
    Write-Host "✓ Added the separate Detailed Report link (isolated from old handlers)." -ForegroundColor Green
  } else {
    Write-Host "⚠ Could not find </body></html> to append the link. Here is the snippet to add manually:" -ForegroundColor Yellow
    Write-Host $link
  }
} else {
  Write-Host "• Detailed report link already present." -ForegroundColor DarkGray
}
