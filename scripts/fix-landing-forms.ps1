# PowerShell 5.1 safe – only rewrites landing-sb.js to avoid clobbering your forms
...
$landingJsPath = Join-Path $WebRoot 'landing-sb.js'
if(-not (Test-Path $landingJsPath)){
  Write-Host "landing-sb.js not found at $landingJsPath. I will still write a fresh one." -ForegroundColor Yellow
}

# New, safer landing-sb.js (no layout/CSS changes)
$landingJs = @'
(function(){
  // tiny helpers
...
Write-Text -Path $landingJsPath -Content $landingJs
Write-Host "Updated: $landingJsPath" -ForegroundColor Green

# sanity: show file sizes/dates
Get-Item $landingJsPath | Format-Table FullName,Length,LastWriteTime -Auto
Write-Host "`nDone. Hard refresh your landing page (Ctrl+F5)." -ForegroundColor Cyan
