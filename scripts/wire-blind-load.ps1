...
$html    = Join-Path $Root $HtmlFile
$jsPath  = Join-Path $Root "bt-load-options.js"

if (!(Test-Path $html)) { Write-Error "Not found: $html"; exit 1 }

# Write loader JS (single-quoted here-string to avoid interpolation issues)
$js = @'
(function(){
...
# Inject a single <script> tag for the loader (idempotent)
$htmlText = Get-Content $html -Raw
$tag = '<script src="./bt-load-options.js" data-url="' + $SupabaseUrl + '" data-key="' + $SupabaseAnonKey + '"></script>'

if ($htmlText -notlike "*bt-load-options.js*") {
  if ($htmlText -match '</body>') {
    $htmlText = [regex]::Replace($htmlText, '</body>', "`n  $tag`n</body>", 'IgnoreCase')
  } else {
    $htmlText += "`n$tag`n"
  }
  $htmlText | Set-Content -Path $html -Encoding UTF8
}

Write-Host "Wired dropdown loader into $HtmlFile. Open it and check Judge/Chip now."
