...
# 1) Remove the two previously injected scripts by id (if present)
#    - wh-force-sum-leaders (force-sum patch)
#    - wh-sauce-leaders-script (sauce card logic)
# (Safe: if they don't exist, nothing is removed.)
...
# 2) Inject a single clean script that renders sums-only and no chip on On-site
$cleanJs = @'
...
# Insert the clean script just before </body> (or append if no </body>)
if ($html -match '(?is)</bod
