...
$dest = Join-Path $root 'onsite.html'
if (Test-Path $dest) {
  Copy-Item -LiteralPath $dest -Destination "$dest.$((Get-Date).ToString('yyyyMMdd_HHmmss')).bak" -Force
}

# Use your provided HTML exactly, with a centered two-button nav under the header.
$html = @'
<!doctype html>
<html lang="en">
<head>
  ...
  <!-- Centered mini-nav (matches landing’s layout) -->
...
<body>
  <header class="header" style="height:2.25in; min-height:2.25in; position:relative; display:flex; align-items:center; justify-content:center;">
    ...
    <h1 style="margin:0; text-align:center;">Whole Hog On-Site Scoring</h1>
    <img class="right-img" src="AL Medallion.png" alt="Logo" />
  </header>

  <!-- Two-button nav directly under the header -->
  <div id="wholehog-nav">
  ...
  <!-- Your existing scripts / Supabase wiring -->
  <script src="onsite.js"></script>
  <script src="supabase-config.js"></script>
  <script src="onsite-sync-teams.js"></script>
</body>
</html>
'@

Set-Content -LiteralPath $dest -Encoding UTF8 -Value $html
Write-Host "✅ onsite.html written from your snippet with centered Home/Leaderboard buttons under the header." -ForegroundColor Green
Start-Process $dest
