...
# ---- Extract HEAD assets (link rel=stylesheet + style blocks) ----
$headMatch = [regex]::Match($landing, '(?is)<head\b[^>]*>(.*?)</head>')
$landingHeadInner = if ($headMatch.Success) { $headMatch.Groups[1].Value } else { "" }

# all stylesheet links
$linkMatches = [regex]::Matches($landingHeadInner, '(?is)<link\b[^>]*rel\s*=\s*["'']stylesheet["''][^>]*>')
$styleMatches = [regex]::Matches($landingHeadInner, '(?is)<style\b[^>]*>.*?</style>')

$landingCssLinks = ($linkMatches | ForEach-Object { $_.Value }) -join "`r`n"
$landingStyleTags = ($styleMatches | ForEach-Object { $_.Value }) -join "`r`n"

# ---- Extract HEADER block exactly as-is ----
...
"@
  ...
  # Replace only the first obvious page title text if we can identify it (h1 or .title).
  $changed = $false
  $newHeader = [regex]::Replace($headerHtml, '(?is)(<h1\b[^>]*>)(.*?)(</h1>)', '${1}Blind Taste${3}', 1)
  if ($newHeader -ne $headerHtml) { $headerHtml = $newHeader; $changed=$true }

  if (-not $changed) {
    $newHeader = [regex]::Replace($headerHtml, '(?is)(<div\b[^>]*class=["''][^"'']*\btitle\b[^"'']*["''][^>]*>)(.*?)(</div>)', '${1}Blind Taste${3}', 1)
    if ($newHeader -ne $headerHtml) { $headerHtml = $newHeader; $changed=$true }
  }

  # If we still couldn't find a title node, we leave header as-is to preserve its structure/buttons/colors.
}

# ---- Our page CSS (only for the scoring form; header styles come from landing head assets) ----
$formCss = @'
:root { --brand:#0d6efd; --ok:#198754; --warn:#ffc107; --bg:#f7f7fb; --ink:#1e1e2a; --muted:#6b7280; --line:#d1d5db; }
*{box-sizing:border-box}
body{margin:0; font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif; color:var(--ink); background:#f7f7fb;}
...
# ---- Full blind.html (includes landing CSS links & inline styles + exact header HTML) ----
$blindHtml = @'
<!doctype html>
<html lang="en">
<head>
...
# ---- Inject assets and header into template ----
$blindHtml = $blindHtml.Replace('__LANDING_LINKS__', $landingCssLinks)
$blindHtml = $blindHtml.Replace('__LANDING_STYLES__', $landingStyleTags)
$blindHtml = $blindHtml.Replace('__FORM_CSS__', $formCss)
$blindHtml = $blindHtml.Replace('__HEADER_HTML__', $headerHtml)
...
Write-Host "✅ Header copied from landing (with landing CSS assets) into $BlindFile"
Write-Host "✅ $BlindFile uses judgesList & chipsList from localStorage for dropdowns"
Write-Host "Open landing.html, Save Lists, then reload blind.html"
