...
$jsPath    = Join-Path $RootPath "bt-local.js"

if (!(Test-Path $blindPath)) {
  Write-Error "Not found: $blindPath"
  exit 1
}
...
# 2) Load HTML
$html = Get-Content $blindPath -Raw

# 3) Clean: remove prior injected scripts/blocks
#   - any prior bt-*.js references
$html = [regex]::Replace($html, '<script[^>]*src\s*=\s*["'']\.\/bt-(wire|load-options|diagnose|local)\.js["''][^>]*>\s*<\/script\s*>', '', 'IgnoreCase')
#   - prior inline wiring/local blocks
$html = [regex]::Replace($html, '<!--\s*BEGIN[^>]*?blind[^>]*?-->.*?<!--\s*END[^>]*?blind[^>]*?-->', '', 'Singleline, IgnoreCase')
$html = [regex]::Replace($html, '<!--\s*BEGIN[^>]*?Local Scoring[^>]*?-->.*?<!--\s*END[^>]*?Local Scoring[^>]*?-->', '', 'Singleline, IgnoreCase')
#   - inline scripts clearly calling supabase or /rest/v1
$html = [regex]::Replace($html, '<script\b[^>]*>[\s\S]*?(supabase|\/rest\/v1)[\s\S]*?<\/script\s*>', '', 'IgnoreCase')

# 4) Trim anything AFTER </html> (sometimes pastes ended up there)
if ($html -match '</html\s*>') { $html = [regex]::Replace($html, '(?is)(</html\s*>).*$', '$1') }

# 5) Write the JS file (single-quoted here-string; closing '@ on its own line, column 1)
$js = @'
...
# 6) Inject script tag before </html> (or append if missing)
$tag = '<script src="./bt-local.js"></script>'

if ($html -notmatch [regex]::Escape($tag)) {
  if ($html -match '</html\s*>') {
    $html = [regex]::Replace($html, '</html\s*>', "`r`n$tag`r`n</html>", 'IgnoreCase')
  } else {
    $html = $html + "`r`n$tag`r`n"
  }
}

# 7) Save HTML
$html | Set-Content -Path $blindPath -Encoding UTF8

Write-Host "Cleaned and wired local scoring. Open $BlindFile, use the setup panel (Pick Judge/Chip/Save) once, then you're good."
