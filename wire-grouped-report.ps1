...
# Remove any older build-report logic to avoid duplicates
$updated = [regex]::Replace($orig, '(?is)<script[^>]*id=["'']wh-build-report-logic["''][^>]*>.*?</script>', '')

# Inject the grouped CSV builder (only)
$logic = @'
...
if ($updated -match '(?is)</body\s*>\s*</html\s*>') {
  $updated = [regex]::Replace($updated, '(?is)</body\s*>\s*</html\s*>', ($logic + "`r`n</body></html>"), 1)
  Set-Content -LiteralPath $Path -Value $updated -Encoding UTF8
  Write-Host "✅ Grouped report wired to the existing button." -ForegroundColor Green
} else {
  Write-Host "ℹ️ Could not find closing </body></html>. No changes written." -ForegroundColor Yellow
}
