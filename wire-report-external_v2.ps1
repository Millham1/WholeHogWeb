...
# --- Prep & backup ---
$root = Split-Path -LiteralPath $Path -Parent
...
# --- 1) Ensure the Build Report button exists at the bottom ---
...
'@
  if ($updated -match '(?is)</body\s*>\s*</html\s*>') {
    ...
    Write-Host "⚠ Could not find </body></html> to append button. Button not added." -ForegroundColor Yellow
  }
} else {
  Write-Host "• Button already present." -ForegroundColor DarkGray
}

# --- 2) Write external JS file with the working grouped report logic ---
...
(function(){
  ...
  function buildGroupedReportCSV(){
    ...
    const csv = rows.map(r => r.map(csvField).join('\\r\\n'===null? ',':',').join(',')).join('\\r\\n'); // safe join
  ...
  function bind(){
    ...
    if (btn) btn.addEventListener('click', buildGroupedReportCSV, { once:false });
  }
...
# --- 3) Inject <script src="wh-report.js"> before </body> if missing ---
# Use double-quoted string and escape quotes with backticks to avoid parser errors
$includePattern = "src=[`"'][^`"']*wh-report\.js[`"']"
if ($updated -notmatch $includePattern) {
...
if ($changed) {
  ...
  Write-Host "ℹ No changes needed to $Path" -ForegroundColor Yellow
}
